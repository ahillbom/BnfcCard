...
void EEPROM_Init(EEPROM_Type *base, const eeprom_config_t *config, uint32_t sourceClock_Hz)
{
    ...
    if ((sourceClock_Hz % (uint32_t)FSL_FEATURE_EEPROM_INTERNAL_FREQ) > ((uint32_t)FSL_FEATURE_EEPROM_INTERNAL_FREQ / 2UL))
    {
        clockDiv += 1UL;
    }
...
{
    uint32_t *addr = (void *)NULL;

    if ((offset % 4UL != 0UL) || (offset > (uint32_t)FSL_FEATURE_EEPROM_SIZE))
    {
        return (uint32_t)kStatus_InvalidArgument;
    }
 ...
 */
        ...
        else if (size >= alignSize)
        {
            ...
            *dst         = (uint32_t)((uint32_t)src[3] << 24UL | (uint32_t)src[2] << 16UL | (uint32_t)src[1] << 8UL | src[0]);
#endif                        /* FSL_FEATURE_EEPROM_TWOBYTES_ALIGNED */
...
status_t EEPROM_WriteRow(EEPROM_Type *base, uint32_t rowNum, uint32_t *data)
{
    ...
    {
        return (int32_t)kStatus_InvalidArgument;
    }
...
status_t EEPROM_WritePage(EEPROM_Type *base, uint32_t pageNum, uint32_t *data)
{
    ...
    if ((pageNum > (uint32_t)FSL_FEATURE_EEPROM_PAGE_COUNT) || (data == (void *)NULL))
    {
        return (int32_t)kStatus_InvalidArgument;
    }
